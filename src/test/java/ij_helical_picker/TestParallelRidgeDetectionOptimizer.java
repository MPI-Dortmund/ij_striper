...
public class TestParallelRidgeDetectionOptimizer {
	...
	public void test_Optimize() {
		...
		*/
		...
		DetectionThresholdRange range = optim.optimize(input, start_params, filament_width, mask_width, GLOBAL_RUNS, LOCAL_RUNS,n_cores, false);
		IJ.log(range.toString());

		assertEquals(0.298, range.getLowerThreshold(),0.01);
		assertEquals(0.872, range.getUpperThreshold(),0.01);
